...
```{r 14-Load Library, include = TRUE}
library(tidyverse)
```
...
```{r 14-String Quote Example, include = TRUE}
...
```{r 14-Rookie Mistake, include = TRUE, eval = FALSE}
...
```{r 14-The magical escape backslash, include = TRUE}
doublequote <- "\"" # or '"'
singlequote <- '\'' # or "'"
print(doublequote)
print(singlequote)
...
```{r 14-But Wait-Theres More, include = TRUE}
doublequote <- "\"" # or '"'
singlequote <- '\'' # or "'"
...
```{r 14-Unicode example, include = TRUE}
x <- "\u00b5"
x
y <- "\U1F44D"
y
...
```{r 14-Combining a string, include = TRUE}
c("one", "two", "three")
```
...
```{r 14-String Length Example, include = TRUE}
...
```{r 14-Combine Strings, include = TRUE}
str_c("x", "y")
str_c("x", "y", "z")
...
```{r 14-Combine Strings using sep, include = TRUE}
str_c("x", "y", sep = ", ")
```
Like many other functions in R, if you have missing data, you can add `NA`.

```{r 14-Strings and NA, include = TRUE}
x <- c("abc", NA)
str_c("|-", x, "-|")
...
```{r 14-recycle shorter vectors, include = TRUE}
...
```{r 14-Collapse String, include = TRUE}
str_c(c("x", "y", "z"))
...
```{r 14-Short Subsetting, include = TRUE}
str_sub("a", 1, 5)
```

You can also use the assignment form of `str_sub()` to modify strings:

```{r 14-Modify Strings, include = TRUE}
...
```{r 14-Local in Capitalization, include = TRUE}
...
```{r 14-Locale Sorting, include = TRUE}
...
```{r 14-Pattern Matching, include=TRUE}
x <- c("apple", "banana", "pear")
str_view(x, "an")
```
...
```{r 14-Match any Character, include = TRUE}
str_view(x, ".a.")
```
...
```{r 14-Escaping the Escape, include = TRUE}
x <- "a\\b"
writeLines(x)
#> a\b
...
```{r 14-match Anything, include=TRUE}
...
```{r 14-Anchor before, include = TRUE}
x <- c("apple pie", "apple", "apple cake")
str_view(x, "apple")
```

```{r 14-Anchor after, include = TRUE}
str_view(x, "^apple$")
```
...
```{r 14-easier readable expressions (a), include = TRUE}
# Look for a literal character that normally has special meaning in a regex
str_view(c("abc", "a.c", "a*c", "a c"), "a[.]c")
```

```{r 14-easeir readable expressions (b), include = TRUE}
str_view(c("abc", "a.c", "a*c", "a c"), ".[*]c")
```

```{r 14-easeir readable expressions (c), include = TRUE}
...
```{r 14-Repetition - 0 or 1, include = TRUE}
x <- "1888 is the longest year in Roman numerals: MDCCCLXXXVIII"
str_view(x, "CC?")
```

```{r 14-Repetition - 1 or more, include = TRUE}
str_view(x, "CC+")
```

```{r 14-Repetition - 0 or more with term, include = TRUE}
str_view(x, 'C[LX]+')
```
...
```{r 14-Repetition - exactly n, include = TRUE}
str_view(x, "C{2}")
```

```{r 14-Repetition - n or more, include = TRUE}
str_view(x, "C{2,}")
```

```{r 14-Repetition - between n and m, include = TRUE}
str_view(x, "C{2,3}")
```
...
```{r 14-Repetition - between n and m - lazy, include = TRUE}
str_view(x, 'C{2,3}?')
```

```{r 14-Repetition - 0 or more with term - lazy, include = TRUE}
str_view(x, 'C[LX]+?')
```
...
```{r 14-Grouping and Backreferences}
...
```{r 14-String Detect, include = TRUE}
x <- c("apple", "banana", "pear")
str_detect(x, "e")
```
...
```{r 14-Think Simple, include = TRUE}
...
```{r 14-Pattern Match, include = TRUE}
...
```{r 14-Using Filter, include = TRUE}
df <- tibble(
  word = words, 
  i = seq_along(word)
)
...
```{r 14-String Count, include = TRUE}
x <- c("apple", "banana", "pear")
str_count(x, "a")
...
```{r 14-String Count with Mutate, include = TRUE}
df %>% 
  mutate(
...
```{r 14-Harvard Sentences, include = TRUE}
...
```{r 14-Select and Extract, Include = TRUE}
...
```{r 14-Sting Extract, include = TRUE}
more <- sentences[str_count(sentences, color_match) > 1]
str_view_all(more, color_match)
```

```{r 14-Extract More, include = TRUE}
str_extract(more, color_match)
```

To get all matches, use `str_extract_all()`.

```{r 14-String Extact All, include = TRUE}
...
```{r 14-Simplify String Extract All, include = TRUE}
...
```{r 14-String Match, include = TRUE}
has_noun %>% 
  str_match(noun)
```
...
```{r 14-String Replace, include = TRUE}
...
```{r 14-Sting Replace Backreference}
sentences %>% 
  str_replace("([^ ]+) ([^ ]+) ([^ ]+)", "\\1 \\3 \\2") %>% 
  head(5)
```
...
```{r 14-Lenght-1 Vector, include = TRUE}
"a|b|c|d" %>% 
  str_split("\\|") %>% 
  .[[1]]
```
...
```{r 14-Splitting, Simplify, include = TRUE}
sentences %>%
  head(5) %>% 
...
```{r 14-Splitting - Maximum, include = TRUE}
...
```{r 14-Splitting - Boundary}
x <- "This is a sentence.  This is another sentence."
str_view_all(x, boundary("word"))
```

```{r 14-Another Boundary split, include = TRUE}
str_split(x, " ")[[1]]
...
```{r 14-Ignore Case Example 1, include = TRUE}
bananas <- c("banana", "Banana", "BANANA")
str_view(bananas, "banana")
```

```{r 14-Ignore Case Example 2, include = TRUE}
str_view(bananas, regex("banana", ignore_case = TRUE))
```

- `multiline = TRUE` allows `^` and `$` to match the start and end of each line rather than the start and end of the complete string.

```{r 14-Mulitline, include = TRUE}
x <- "Line 1\nLine 2\nLine 3"
...
```{r 14-stringi example, include = TRUE}
...
```{r 14-String View All, include = TRUE}
x <- "This is a sentence."
str_view_all(x, boundary("word"))
```

```{r 14-String View All with Boundary, include = TRUE}
