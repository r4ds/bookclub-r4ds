...
- We are going to learn how to create logical vectors using the comparism operators

- We are going to learn how to use Boolean algebra to combine different logical vectors as well as some useful summaries.

- We are going to learn how to use **if_else()** and **case_when()** to make conditional changes powered by logical vectors.

## Introduction

**Logical vectors** are the simplest type of vector because each element can only be one of three possible values: **TRUE**, **FALSE**, and **NA**. It’s relatively rare to find logical vectors in your raw data, but we’ll create and manipulate in the course of almost every analysis.
...
## Comparisons
A very common way to create a logical vector is via a numeric comparison with **<**, **<=**, **>**, **>=**, **!=**, and **==**. So far, we’ve mostly created logical variables transiently within filter() 
— they are computed, used, and then thrown away. For example, the following filter finds all daytime departures that leave roughly on time:
...
It’s useful to know that this is a shortcut and we can explicitly create the underlying logical variables with mutate():
...
What’s going on? Computers store numbers with a fixed number of decimal places so there’s no way to exactly represent **1/49** or **sqrt(2)** and subsequent computations will be very slightly off. We can see the exact values by calling **print()** with the the **digits argument**:

```{r}
print(x, digits = 16)
```

We can see why R defaults to rounding these numbers; they really are very close to what you expect.

Now that we’ve seen **why ==** is failing, what can you do about it? One option is to use **dplyr::near()** which ignores small differences:
...
So if we want to find all flights with **dep_time** is missing, the following code doesn’t work because **dep_time == NA** will yield a **NA** for every single row, and **filter()** automatically drops missing values:
...
Once you have multiple logical vectors, you can combine them together using Boolean algebra. In R, & is “and”, | is “or”, and ! is “not”, and xor() is exclusive or2. Figure 12.1 shows the complete set of Boolean operations and how they work.
...
```
We might be tempted to write it like you’d say in English: “find all flights that departed in November or December”:
...
When you use a logical vector in a numeric context, TRUE becomes 1 and FALSE becomes 0. This makes sum() and mean() very useful with logical vectors because sum(x) will give the number of TRUEs and mean(x) the proportion of TRUEs.
...
There’s one final use for logical vectors in summaries: you can use a logical vector to filter a single variable to a subset of interest. This makes use of the base [ (pronounced subset) operator, which you’ll learn more about in Section 26.1.
...
The definition of a logical vector is simple because each value must be either **TRUE**, **FALSE**, or **NA**. But logical vectors provide a huge amount of power. In this chapter, we learned how to create logical vectors with >, <, <=, >=, ==, !=, and **is.na()**, how to combine them with !, &, and |, and how to summarize them with **any()**, **all()**, **sum()**, and **mean()**. We also learned the powerful **if_else()** and **case_when()** that allow us to return values depending on the value of a logical vector.

We’ll see logical vectors again and in the following chapters. For example in Chapter 14 we’ll learn about **str_detect(x, pattern)** which returns a logical vector that’s TRUE for the elements of x that match the pattern, and in **Chapter 17** we’ll create logical vectors from the comparison of dates and times. But for now, we’re going to move onto the next most important type of vector: numeric vectors.
