...
```{r 16-Load Library, include = TRUE}
library(tidyverse)
```
...
```{r 16-String Quote Example, include = TRUE}
...
```{r 16-Rookie Mistake, include = TRUE, eval = FALSE}
...
```{r 16-The magical escape backslash, include = TRUE}
doublequote <- "\"" # or '"'
singlequote <- '\'' # or "'"
print(doublequote)
print(singlequote)
...
```{r 16-But Wait-Theres More, include = TRUE}
doublequote <- "\"" # or '"'
singlequote <- '\'' # or "'"
...
```{r 16-Unicode example, include = TRUE}
x <- "\u00b5"
x
y <- "\U1F44D"
y
...
```{r 16-Combining a string, include = TRUE}
c("one", "two", "three")
```
...
```{r 16-String Length Example, include = TRUE}
...
```{r 16-Combine Strings, include = TRUE}
str_c("x", "y")
str_c("x", "y", "z")
...
```{r 16-Combine Strings using sep, include = TRUE}
str_c("x", "y", sep = ", ")
```
Like many other functions in R, if you have missing data, you can add `NA`.

```{r 16-Strings and NA, include = TRUE}
x <- c("abc", NA)
str_c("|-", x, "-|")
...
```{r 16-recycle shorter vectors, include = TRUE}
...
```{r 16-Collapse String, include = TRUE}
str_c(c("x", "y", "z"))
...
```{r 16-Short Subsetting, include = TRUE}
str_sub("a", 1, 5)
```

You can also use the assignment form of `str_sub()` to modify strings:

```{r 16-Modify Strings, include = TRUE}
...
```{r 16-Local in Capitalization, include = TRUE}
...
```{r 16-Locale Sorting, include = TRUE}
...
```{r 16-Pattern Matching, include=TRUE}
x <- c("apple", "banana", "pear")
str_view(x, "an")
```
...
```{r 16-Match any Character, include = TRUE}
str_view(x, ".a.")
```
...
```{r 16-Escaping the Escape, include = TRUE}
x <- "a\\b"
writeLines(x)
#> a\b
...
```{r 16-match Anything, include=TRUE}
...
```{r 16-Anchor before, include = TRUE}
x <- c("apple pie", "apple", "apple cake")
str_view(x, "apple")
```

```{r 16-Anchor after, include = TRUE}
str_view(x, "^apple$")
```
...
```{r 16-easier readable expressions (a), include = TRUE}
# Look for a literal character that normally has special meaning in a regex
str_view(c("abc", "a.c", "a*c", "a c"), "a[.]c")
```

```{r 16-easeir readable expressions (b), include = TRUE}
str_view(c("abc", "a.c", "a*c", "a c"), ".[*]c")
```

```{r 16-easeir readable expressions (c), include = TRUE}
...
```{r 16-Repetition - 0 or 1, include = TRUE}
x <- "1888 is the longest year in Roman numerals: MDCCCLXXXVIII"
str_view(x, "CC?")
```

```{r 16-Repetition - 1 or more, include = TRUE}
str_view(x, "CC+")
```

```{r 16-Repetition - 0 or more with term, include = TRUE}
str_view(x, 'C[LX]+')
```
...
```{r 16-Repetition - exactly n, include = TRUE}
str_view(x, "C{2}")
```

```{r 16-Repetition - n or more, include = TRUE}
str_view(x, "C{2,}")
```

```{r 16-Repetition - between n and m, include = TRUE}
str_view(x, "C{2,3}")
```
...
```{r 16-Repetition - between n and m - lazy, include = TRUE}
str_view(x, 'C{2,3}?')
```

```{r 16-Repetition - 0 or more with term - lazy, include = TRUE}
str_view(x, 'C[LX]+?')
```
...
```{r 16-Grouping and Backreferences}
...
```{r 16-String Detect, include = TRUE}
x <- c("apple", "banana", "pear")
str_detect(x, "e")
```
...
```{r 16-Think Simple, include = TRUE}
...
```{r 16-Pattern Match, include = TRUE}
...
```{r 16-Using Filter, include = TRUE}
df <- tibble(
  word = words, 
  i = seq_along(word)
)
...
```{r 16-String Count, include = TRUE}
x <- c("apple", "banana", "pear")
str_count(x, "a")
...
```{r 16-String Count with Mutate, include = TRUE}
df %>% 
  mutate(
...
```{r 16-Harvard Sentences, include = TRUE}
...
```{r 16-Select and Extract, Include = TRUE}
...
```{r 16-Sting Extract, include = TRUE}
more <- sentences[str_count(sentences, color_match) > 1]
str_view_all(more, color_match)
```

```{r 16-Extract More, include = TRUE}
str_extract(more, color_match)
```

To get all matches, use `str_extract_all()`.

```{r 16-String Extact All, include = TRUE}
...
```{r 16-Simplify String Extract All, include = TRUE}
...
```{r 16-String Match, include = TRUE}
has_noun %>% 
  str_match(noun)
```
...
```{r 16-String Replace, include = TRUE}
...
```{r 16-Sting Replace Backreference}
sentences %>% 
  str_replace("([^ ]+) ([^ ]+) ([^ ]+)", "\\1 \\3 \\2") %>% 
  head(5)
```
...
```{r 16-Lenght-1 Vector, include = TRUE}
"a|b|c|d" %>% 
  str_split("\\|") %>% 
  .[[1]]
```
...
```{r 16-Splitting, Simplify, include = TRUE}
sentences %>%
  head(5) %>% 
...
```{r 16-Splitting - Maximum, include = TRUE}
...
```{r 16-Splitting - Boundary}
x <- "This is a sentence.  This is another sentence."
str_view_all(x, boundary("word"))
```

```{r 16-Another Boundary split, include = TRUE}
str_split(x, " ")[[1]]
...
```{r 16-Ignore Case Example 1, include = TRUE}
bananas <- c("banana", "Banana", "BANANA")
str_view(bananas, "banana")
```

```{r 16-Ignore Case Example 2, include = TRUE}
str_view(bananas, regex("banana", ignore_case = TRUE))
```

- `multiline = TRUE` allows `^` and `$` to match the start and end of each line rather than the start and end of the complete string.

```{r 16-Mulitline, include = TRUE}
x <- "Line 1\nLine 2\nLine 3"
...
```{r 16-stringi example, include = TRUE}
...
```{r 16-String View All, include = TRUE}
x <- "This is a sentence."
str_view_all(x, boundary("word"))
```

```{r 16-String View All with Boundary, include = TRUE}
