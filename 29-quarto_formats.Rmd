...
```{r eval=FALSE}
#You can  produce multiple types of output since the `output_format` argument can take a list of values
...
format:
  ...
  pdf: default
  docx: default
  #Note the special syntax (`pdf: default` and `docx: default`) if you don’t want to override any default options.
```
...
for example, two options for including interactivity in your Quarto documents: htmlwidgets and Shiny.
  - **htmlwidgets**, R functions that produce interactive HTML visualizations
  - **shiny**, a package that allows you to create interactivity using R code
...
- - [Awesome Quarto Extensions](https://m.canouil.dev/quarto-extensions/): Quarto extensions listing from Awesome Quarto.
