...
When solving data manipulation challenges with `dplyr`, you will in general have the following structure:
...
### Pipe
The pipe symbol `|>` in R is used to combine different functions in a sequence, where the output of one function becomes the input of the next function. It is suggested to pronounce pipe as "then".
...
```
The sentence "in plain English" would be:

"Take the flights dataset, then filter the flights that have their destination (dest) as 'IAH', then group these filtered results by year, month, and day, and then for each group, calculate the average arrival delay (arr_delay), ignoring the missing values."
...
## Common mistakes
Use `=` instead of `==` when testing for equality. `filter()` will let you know when this happens:
...
```{r 04-arrange1}
# sorts by the departure time, the earliest years first, then within a year the earliest months, etc
flights |> 
...
## Distinct
`distinct()` finds all the unique rows in a dataset, it primarily operates on the rows. To find the distinct combination of some variables you can also optionally supply column names.
...
# keep other columns when filtering for unique rows, use the .keep_all = TRUE option.

flights |> 
  distinct(origin, dest, .keep_all = TRUE)
```

To find the number of occurrences instead, swap distinct() for count(), and with the sort = TRUE argument you can arrange them in descending order of number of occurrences.
