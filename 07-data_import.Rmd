...
- We use the `readr` package which is part of the `tidyverse`. 
...
- We can read CSV files into R using `readr::read_csv()`. 
  - The first argument is the path of the file:

#### Pulling the file from the data folder and using the `here` package {-}
...
- You can transform the data once you read it in. This can includes addressing issues such as:
...
- In the `favourite.food` column, let's include `N/A` to be recognized as a real `NA`.
  - By default, `read_csv()` only recognizes empty strings (`""`) in this dataset as `NA`s. 
...
- The `Student ID` and `Full Name` columns are surrounded by backticks because they contain spaces (i.e. are **non-syntactic**:
