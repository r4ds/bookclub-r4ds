...
- This is all about conditions on data values that return TRUE or FALSE.
- Useful in filtering, mutating and summarizing.
...
- Operations that don't change vector lengths
  ...
  - conditional transformations: generate a vector from hierarchical conditions (logical vectors ~ comparisons)
- Subsetting vectors
...
In most cases an `NA` value (vector element) is seen as 'missing so we can't (always) know the outcome':

- `NA`in comparisons will always return `NA`.
  - so `x == NA` will just return `NA` for all elements.
  - check for missing values with **`is.na()`**: `TRUE` for missing values and `FALSE` for everything else
  
- `NA` in boolean algebra: sometime the outcome is known, sometimes not (hence `NA`):
...
Hierarchy of logical vectors (~ comparisons, conditions) leads to a vector where each element is determined by the value of one or multiple conditions.

- one condition: use `if_else()`
  - `if_else(condition, true, false, missing = NULL)`
  
- multiple (hierarchical) conditions: use `case_when(..., .default = NULL)`.
  - the _first_ condition that is `TRUE` for an element determines the value that element.
...
- This uses just base R!
- Provide a logical vector in the brackets (obtained by one of the previous techniques; often comparison)
...
## Summarizing logical vectors (~ summarizing comparisons)
