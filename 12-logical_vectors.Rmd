...
```{r 12-load_tidyverse, message=FALSE, eval=FALSE} 
library(tidyverse)
```
...
```{r 12-logical_vector, eval=FALSE}
1:10 %% 3 == 0
...
```{r 12-numeric_vector, eval=FALSE}
typeof(1)
...
```{r 12-doubles_vector, eval=FALSE}
(x3 <- sqrt(2) ^ 2)
...
```{r 12-checking_memory_character, eval=FALSE}
...
```{r 12-atomic_vec_missing_value, eval=FALSE}
NA            # logical
...
```{r 12-diff_finite_inifinite, eval=FALSE}
...
```{r 12-info_dplyr_near, eval=FALSE}
dplyr::near()
```
...
```{r 12-possible_values_numeric, eval=FALSE}
help(integer)
help(double)
```
...
```{r 12-max_integer_value, eval=FALSE}
.Machine$integer.max
...
```{r 12-max_idouble_value, eval=FALSE}
.Machine$double.xmax
```
...
```{r 12-rounding_double, eval=FALSE}
tibble(
...
```{r 12-parse_integer_nonNumeric, eval=FALSE}
parse_integer(c("1000", "$1,000", "10.00"))
```

```{r 12-parse_numeric_nonNumeric, eval=FALSE}
...
```{r 12-implicit_coerce_logical_to_numeric, eval=FALSE}
...
```{r 12-implicit_coerce_numeric_to_logical, eval=FALSE}
if (length(x)) {
  # do something
}
```
...
```{r 12-coerce_vector_with_multiple_types, eval=FALSE}
...
```{r 12-adding_vectors_with_lengths_differ, eval=FALSE}
1:10 + 1:2
```
...
```{r 12-adding_vec_with_lengths_differ_not_multiple, eval=FALSE}
1:10 + 1:3
```
...
```{r 12-adding_cols_with_lengths_differ, eval=FALSE}
tibble(x = 1:4, y = 1:2)
...
```{r 12-naming_vec_at_creation, eval=FALSE}
c(x = 1, y = 2, z = 4)
```

- Or, with `purrr::set_names()`:

```{r 12-naming_vec_using_purrr, eval=FALSE}
...
```{r 12-subset_vec_positive_integers, eval=FALSE}
...
```{r 12-subset_vec_positive_integers1, eval=FALSE}
x[c(1, 1, 5, 5, 5, 2)]
```

Negative values drop the elements at the specified positions:

```{r 12-subset_vec_negative_integers, eval=FALSE}
x[c(-1, -3, -5)]
```

It’s an error to mix positive and negative values:

```{r 12-subset_vec_positive_and negative_integers, eval=FALSE}
x[c(1, -1)]
```

The error message mentions subsetting with zero, which returns no values:

```{r 12-subset_vec_zero, eval=FALSE}
x[0]
```
...
```{r 12-subset_vec_logical, eval=FALSE}
...
```{r 12-subset_named_vec_with_character_vec, eval=FALSE}
...
```{r 12-subset_named_vec_with_character_vec1, eval=FALSE}
x[c("xyz", "xyz", "def", "def")]
```
...
```{r 12-ex20.4.1_is.na_is_is.finite, eval=FALSE}
...
```{r 12-ex20.4.2_is_atomic, eval=FALSE}
is.atomic(1:10)
is.atomic(list(a = 1))
```
...
```{r 12-ex20.4.2_is_atomic_attr, eval=FALSE}
is.atomic(x)
```
...
```{r 12-ex20.4.3_setNames_fn, eval=FALSE}
...
```{r 12-ex20.4.3_set_names_fn_4, eval=FALSE}
...
```{r 12-ex20.4.3_set_names_fn_5, eval=FALSE}
...
```{r 12-ex20.4.3_set_names_fn_6, eval=FALSE}
...
```{r 12-ex20.4.3_setNames_fn_7, eval=FALSE}
setNames(1:4, c("a", "b"))
```
...
```{r 12-lists_with_mix_objects, eval=FALSE}
...
```{r 12-lists_within_lists, eval=FALSE}
...
```{r 12-visual_rep_lists, eval=FALSE}
x1 <- list(c(1, 2), c(3, 4))
...
```{r 12-subsetting_named_lists, eval=FALSE}
a$a
...
```{r 12-ex20.5.1_draw_lists, eval = FALSE}
...
```{r 12-ex20.5.2_diff_lists_tibbles, eval=FALSE}
...
```{r 12-method_generic_fn, eval=FALSE}
methods("as.Date")
```
...
```{r 12-implementation_of_method_default, eval=FALSE}
getS3method("as.Date", "default")
```

```{r 12-implementation_of_method_numeric, eval=FALSE}
...
```{r 12-augmented_factors, eval=FALSE}
...
```{r 12-augmented_dates, eval=FALSE}
...
```{r 12-augmented_date_time, eval=FALSE}
...
```{r 12-augmented_date_time_tzone, eval=FALSE}
...
```{r 12-augmented_date_time_POSIXlt, eval=FALSE}
...
```{r 12-augmented_tibbles, eval=FALSE}
...
```{r 12-ex20.7.2_1_scalar, eval=FALSE}
tibble(x = 1, y = 1:5)
```

The "scalar" 1 is recycled to the length of the longer vector.

```{r 12-ex20.7.2_2_tibbles_cols_diff_lengths, eval=FALSE}
tibble(x = 1:3, y = 1:4)
```
...
```{r 12-ex20.7.3_lists_within_tibbles, eval=FALSE}
